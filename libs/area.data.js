(function(AREAS){
    ...
    for(var v in cities){
        for(var n=0;n<cities[v].length;n++){
            cities[v][n].children = locals[cities[v][n].value];
        }
    }
    for(var i=0;i<provinces.length;i++){
        provinces[i].children = cities[provinces[i].value];
        if(locals[provinces[i].value]){
            provinces[i].children = locals[provinces[i].value];
        }
    }

    window.AREAS = {
        list: provinces,
        getLabel: function(areaId){
            var province = '', city = '', local = '';
            var provinceId = areaId[0];
            var cityId = areaId[1];

            for(var i=0;i<provinces.length;i++){
                if(provinces[i].value == provinceId){
                    province = provinces[i].label;
                }
            }

            if(areaId.length == 2){
                if(areaId[0] == 110000 || areaId[0] ==  120000 || areaId[0] ==  310000 || areaId[0] == 500000){
                    var _locals = locals[provinceId];
                }else{
                    var _locals = cities[provinceId];
                }
                    for(var i=0;i<_locals.length;i++){
                        if(_locals[i].value == cityId){
                            local = _locals[i].label;
                        }
                    }
                    return [province, local]   
            }

            var localId = areaId[2];
            var _cities = cities[provinceId];
            for(var i=0;i<_cities.length;i++){
                if(_cities[i].value == cityId){
                    city = _cities[i].label;
                }
            }

            var _locals = locals[cityId];
            for(var i=0;i<_locals.length;i++){
                if(_locals[i].value == localId){
                    local = _locals[i].label;
                }
            }
            return [province, city, local];
        }
    };
})(window.AREAS = {});
